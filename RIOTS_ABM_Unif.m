function RIOTS_ABM_Unif(Ttot,P)
...
m0r=0.05; % Initial rioter density (for spatial-uniformity)
...
K=m0r+m0b;
    ...
    Def=[1 1 1 1 1]; % DEFINE INDIVIDUAL DEFECTION RATES HERE
    
%     Rec=0:0.25:1;
%     Def=(m0r+m0b)/4*ones(1,5);
...
%        QBend=QB(j+1); % Updating number of bystanders after step
    ...
    figure(15)
     plot3(YY(CR==1),XX(CR==1),2*CR(CR==1),'ro','markersize',5,'markerfacecolor','r')
       ...
       hold on
    plot3(YY(CB==1),XX(CB==1),CB(CB==1),'bo','markersize',5,'markerfacecolor','b')
    ...
    QR=QR(U);
    QB=QB(U);
    G=griddedInterpolant(tau,QB/(X*Y),'nearest'); % Interpolate over one simulation
    QBi(p,:)=G(t); % Store interpolation vector for p-th simulation
    G2=griddedInterpolant(tau,QR/(X*Y),'nearest'); % Interpolate over one simulation
...
figure(50)
hold off
plot(t,QQr,'r',t,QQb,'b','LineWidth',2) % Plot averaged total densities
hold on
axis([0 Ttot 0 m0r+m0b])
%errorbar(t(1:50:end),QQr(1:50:end),QQrs(1:50:end),'r')
%errorbar(t(1:50:end),QQb(1:50:end),QQbs(1:50:end),'b')

set(gca,'FontSize',20)
RIOTS_contlim_spatiallyuniform(Rec,Def,m0r,m0b,Ttot)